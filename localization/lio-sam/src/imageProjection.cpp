...
private:
  ...
  rclcpp::Publisher<autoware_localization_msgs::msg::CloudInfo>::SharedPtr pubLaserCloudInfo;
...
public:
  ...
  : ParamServer("lio_sam_imageProjection", options), deskewFlag(0)
  {
    ...
    subImu = create_subscription<sensor_msgs::msg::Imu>(
      imuTopic, qos_imu, std::bind(&ImageProjection::imuHandler, this, std::placeholders::_1),
      imuOpt);
    ...
    subLaserCloud = create_subscription<sensor_msgs::msg::PointCloud2>(
      ...
      std::bind(&ImageProjection::cloudHandler, this, std::placeholders::_1), lidarOpt);

    pubExtractedCloud =
      create_publisher<sensor_msgs::msg::PointCloud2>("lio_sam/deskew/cloud_deskewed", 1);
    pubLaserCloudInfo = create_publisher<autoware_localization_msgs::msg::CloudInfo>("lio_sam/deskew/cloud_info", qos);
...
//    }
  ...
  void imuDeskewInfo()
  {
    ...
    for (int i = 0; i < (int)imuQueue.size(); ++i) {
      ...
      if (currentImuTime <= timeScanCur)
        imuRPY2rosRPY(
          &thisImuMsg, &cloudInfo.imu_roll_init, &cloudInfo.imu_pitch_init,
  ...
  PointType deskewPoint(PointType * point, double relTime)
  {
    ...
    PointType newPoint;
    newPoint.x = transBt(0, 0) * point->x + transBt(0, 1) * point->y + transBt(0, 2) * point->z +
                 transBt(0, 3);
    newPoint.y = transBt(1, 0) * point->x + transBt(1, 1) * point->y + transBt(1, 2) * point->z +
                 transBt(1, 3);
    newPoint.z = transBt(2, 0) * point->x + transBt(2, 1) * point->y + transBt(2, 2) * point->z +
                 transBt(2, 3);
